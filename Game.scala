...
class Game(val player1: Player,
  ...
  def nextOpenEnd(currentPlayer: Player): String = pickNextOrNot(currentPlayer).unsafeRun() match {
  ...
  def wrongMove(currentPlayer: Player): String = pickTile(currentPlayer, s"dominogame:~ Tile ends dismatch! Try again!\n")
  ...
  def checkReceivedIndex(validatedNum: Validated[String, Tile], currentPlayer: Player): String = validatedNum match {
    case Invalid(e) => pickTile(currentPlayer, "dominogame:~ " + e + "\n")
    case Valid(t) => analysingMove(t, currentPlayer)
  }

  def pickTile(currentPlayer: Player, message: String): String = toInt(readCommand(message).unsafeRun()) match {
    case None => pickTile(currentPlayer, "")
    case Some(num) => checkReceivedIndex(currentPlayer.drawTileFromPile(num), currentPlayer)
  }

  def identifyCommand(currentPlayer: Player, message: String):  String = readCommand(message).unsafeRun() match {
  ...
  def pickMoveLoop(currentPlayer: Player, message: String): Boolean = identifyCommand(currentPlayer, message) match {
    case "" => true
  ...
  def playerScheduler( quit: Boolean, currentPlayer: Int): IO[Unit] = if (player1.pile.length > 0 && player2.pile.length > 0 && !quit) {
    currentPlayer match {
